...
//  Almonaseq
//
//  Created by Nada El Hakim on 3/15/18.
...
class ContactVC: UITableViewController,  MFMessageComposeViewControllerDelegate {
    ...
    @IBAction func handleHomePhoneCall(_ sender: Any) {
        var homePhone = "013594449";
        guard let number = URL(string: "tel://" + homePhone) else { return }
        if #available(iOS 10.0, *) {
            UIApplication.shared.open(number)
        } else {
            // Fallback on earlier versions
