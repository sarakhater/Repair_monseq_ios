...
public class HWSwiftyViewPager: UICollectionView, UICollectionViewDelegate {
    ...
    override public func layoutSubviews() {
        ...
        if !self.frame.equalTo(self.beforeFrame) {
            ...
            self.flowLayout.itemSize = CGSize.init(width: widthNew, height: heightNew);
            ...
            let targetX = self.getOffSetFromPage(pageNum: self.selectedPageNum, scrollView: self)
            self.contentOffset = CGPoint.init(x: targetX, y: 0);
            self.layoutIfNeeded()
        }
    ...
    public func scrollViewWillEndDragging(scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        self.pageSelectedDelegate?.pagerDidSelecedPage(selectedPage: selectedPageNum)
        
        point.x = self.getOffSetFromPage(pageNum: self.selectedPageNum, scrollView: scrollView);
    ...
    public func setPage(pageNum pageNum:Int, isAnimation:Bool){
        ...
        let offset = getOffSetFromPage(pageNum: pageNum, scrollView: self)
        self.setContentOffset(CGPoint.init(x: offset, y: 0), animated: isAnimation)
        self.selectedPageNum = pageNum
        self.pageSelectedDelegate?.pagerDidSelecedPage(selectedPage: pageNum)
    }
